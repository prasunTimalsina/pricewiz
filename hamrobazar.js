  ...
  while (idleCounter < maxIdle && collectedItems.length < maxItems) {
    const newItems = await page.evaluate(() => {
      ...
      return Array.from(nodes).map(node => {
        ...
        const price = priceContainer?.querySelector('span.regularPrice')?.innerText || null;
...
export async function scrapeHamrobazaarPrice(Hurl, lowPrice, highPrice) {
  ...
  // Wait for price filter form
  await page.waitForSelector('.filter--form.show', { timeout: 30000 });

  // Set min price
  await page.focus('input[name="priceFrom"]');
  await page.keyboard.down('Control');
  await page.keyboard.press('A');
  await page.keyboard.up('Control');
  await page.keyboard.press('Backspace');
  await page.type('input[name="priceFrom"]', String(lowPrice), { delay: 100 });

  // Set max price
  await page.focus('input[name="priceTo"]');
  await page.keyboard.down('Control');
  await page.keyboard.press('A');
  await page.keyboard.up('Control');
  await page.keyboard.press('Backspace');
  await page.type('input[name="priceTo"]', String(highPrice), { delay: 100 });

  // Click the filter button
  await page.waitForSelector('.form-item.form-item--filterBtn .btn', { visible: true });
  await page.evaluate(() => {
    const btn = document.querySelector('.form-item.form-item--filterBtn .btn');
    if (btn) btn.click();
  });
